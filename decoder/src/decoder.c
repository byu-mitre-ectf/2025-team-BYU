...
void randomSleep(void) {
    ...
    while (1) { // Loop for random wait
        uint32_t current_time = *(real_time_clock + SUBSEC_CTR_OFFSET);

        // edge case was super high loop cap or super low loop cap, so those cases have been covered by the or statements
        if ((!rollover && (current_time >= loop_cap || current_time < SLEEP_MASK)) // Delay check
          || (rollover && current_time < SLEEP_MASK && (current_time <= loop_cap || current_time <= loop_cap + 0x40))) { // Rollover check
            break;
        }
    }
